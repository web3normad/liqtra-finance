...
export class BlockchainService {
  ...
  async getUserProtocolBalance(userAddress: string, protocolAddress: string): Promise<bigint> {
    try {
      const balance = await this.vaultContract.getUserProtocolBalance(userAddress, protocolAddress);
      return balance;
    } catch (error) {
      logger.error('Error reading protocol balance', { user: userAddress, protocol: protocolAddress, error });
      return 0n;
    }
  }
  ...
  async getUserTotalEarned(userAddress: string): Promise<bigint> {
    try {
      ...
      logger.error('Error reading total earned', { user: userAddress, error });
      return 0n;
    }
  }
